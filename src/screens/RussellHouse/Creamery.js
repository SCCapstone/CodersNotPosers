...
const Creamery = ({navigation}) => {
  ...
  const [menuType, setMenuType] = useState('Ice-creams');
  ...
  useEffect(() => {
    ...
    const docRef = favoritesRef.doc('Creamery');
    docRef.get().then((doc) => {
      if (doc.exists) {
  ...
  const toggleFavorite = async () => {
    ...
    const docRef = firebase.firestore().collection('Favorites').doc(user.uid).collection('Restaurants').doc('Creamery');
  ...
  const renderCategory = () => {
    switch (menuType) {
      case 'Ice-creams':
        return (
          <FlatList
            data={menuData.icecream[0].items.map((item, index) => ({
              item,
              price: menuData.icecream[0].prices[index],
            }))}
      ...
      case 'Topping':
        return (
          <FlatList
            data={menuData.topping[0].items.map((item, index) => ({
              item,
              price: menuData.topping[0].prices[index],
            }))}
            keyExtractor={(item) => item.item}
            renderItem={renderItem}
    ...
    <View style= {styles.container}>
            ...
            <Image source={home} 
                ...
                <View style={styles.profileNameContainer}>
                  <Text style={styles.nameText}>Carolina Creamery</Text>
                </View>
                </TouchableOpacity>   
